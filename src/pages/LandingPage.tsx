...
import { Star, ShieldCheck, Zap, Flame, Truck, PhoneCall, Gift, ArrowLeft, ArrowRight } from 'lucide-react';
  ...
  return (
    <div className="min-h-screen w-screen font-sans bg-gray-50 dark:bg-[#0A0A11] text-gray-900 dark:text-white transition-colors duration-300 flex flex-col">
      ...
      <div className="flex flex-1 w-full max-w-[1600px] mx-auto relative">
        ...
        {/* PAGE CONTENT */}
        <main className="flex-1 min-w-0 px-2 sm:px-6 lg:px-8 py-4 sm:py-6 space-y-4 sm:space-y-8 transition-all duration-300">

          {/* HERO CAROUSEL */}
          <div
            className="relative group w-full rounded-xl sm:rounded-2xl overflow-hidden aspect-[16/7] sm:aspect-[16/6] cursor-grab"
            ...
            {HERO_SLIDES.map((slide, idx) => (
              ...
              >
                ...
                <div className="absolute inset-0 flex flex-col justify-center px-6 sm:px-12 z-10">
                  <span className={`${slide.tagColor} text-white text-[9px] sm:text-xs font-bold px-2 py-1 rounded w-fit mb-2 sm:mb-3 tracking-widest`}>
                    {slide.tag}
                  </span>
                  <h2 className="text-white font-black text-xl sm:text-4xl lg:text-5xl leading-tight max-w-lg whitespace-pre-line drop-shadow-lg">
                    {slide.title}
                  </h2>
                  <p className="text-white/80 text-[10px] sm:text-sm mt-2 sm:mt-3 max-w-sm leading-relaxed hidden sm:block">
                    {slide.desc}
                  </p>
                  <button className="mt-3 sm:mt-5 bg-[#6324E2] hover:bg-[#7c3aed] text-white text-[10px] sm:text-sm font-bold px-4 sm:px-6 py-2 sm:py-2.5 rounded-lg w-fit transition-colors shadow-lg">
                    {slide.button}
                  </button>
                </div>
              </div>
            ...
            <div className="absolute bottom-2 sm:bottom-4 left-0 right-0 flex justify-center gap-1.5 sm:gap-2 z-20">
              {HERO_SLIDES.map((_, idx) => (
                <button
                  ...
                  className={`h-1.5 sm:h-2.5 rounded-full transition-all drop-shadow-md ${currentSlide === idx ? 'bg-white w-4 sm:w-8' : 'bg-white/50 hover:bg-white/80 w-1.5 sm:w-2.5'}`}
                />
              ))}
            </div>
          </div>
          ...
          <div>
            <div className="flex items-center justify-between bg-red-800 rounded-t-lg sm:rounded-t-xl px-3 sm:px-4 py-2 sm:py-3">
              <h2 className="text-xs sm:text-base font-bold text-white flex items-center gap-1 sm:gap-2">
                <Zap className="text-yellow-300 w-3 h-3 sm:w-[18px] sm:h-[18px]" fill="currentColor"/> Flash Sales
              </h2>
                    ...
                    <div className="absolute bottom-1 left-1 flex flex-wrap gap-1 z-30">
                      {product.tag === 'Verified' && (
                        <span className="bg-emerald-600 text-white text-[7px] sm:text-[8px] font-bold px-1 py-0.5 rounded flex items-center gap-0.5">
                          <ShieldCheck className="w-2 h-2 sm:w-[10px] sm:h-[10px]" />
                        </span>
                      )}
                    </div>
                  </div>
