...
import { 
  Zap, Clock, Star, ShieldCheck, Flame, Truck, PhoneCall, Gift,
  ArrowDownLeft, ArrowUpRight, Lock, ChevronLeft, ChevronRight, Grid, List, Heart
} from 'lucide-react';
...
const RECENT_TRANSACTIONS = [
  { id: "TXN-4821", type: "deposit", label: "Wallet Top-up", amount: "+$500.00", date: "Today, 10:42 AM", color: "text-emerald-500", bg: "bg-emerald-50 dark:bg-emerald-500/10", icon: <ArrowDownLeft className="w-3.5 h-3.5" /> },
  { id: "TXN-4820", type: "payment", label: "Order #ORD-98211", amount: "-$1,450.00", date: "Today, 9:15 AM", color: "text-red-500", bg: "bg-red-50 dark:bg-red-500/10", icon: <ArrowUpRight className="w-3.5 h-3.5" /> },
  { id: "TXN-4819", type: "escrow", label: "Escrow Released", amount: "+$299.00", date: "Jul 23, 2:30 PM", color: "text-emerald-500", bg: "bg-emerald-50 dark:bg-emerald-500/10", icon: <ArrowDownLeft className="w-3.5 h-3.5" /> },
  { id: "TXN-4818", type: "escrow_hold", label: "Escrow Hold", amount: "-$2,100.00", date: "Jul 22, 11:00 AM", color: "text-orange-500", bg: "bg-orange-50 dark:bg-orange-500/10", icon: <Lock className="w-3.5 h-3.5" /> },
];
...
const DashboardPage = () => {
  ...
  return (
    <div className="min-h-screen w-screen font-sans bg-gray-50 dark:bg-[#0A0A11] text-gray-900 dark:text-white transition-colors duration-300 flex flex-col">
      ...
      <div className="flex flex-1 w-full max-w-[1600px] mx-auto relative">
        ...
        {/* SLIDE-IN SIDEBAR */}
        <div className={`hidden lg:block sticky top-[130px] h-[calc(100vh-130px)] transition-all duration-300 ease-in-out shrink-0 overflow-hidden
        ...
        <div className="flex-1 min-w-0 px-2 sm:px-6 lg:px-8 py-4 sm:py-6 lg:py-10 transition-all duration-300">
          <div className="flex flex-col xl:flex-row gap-4 sm:gap-6 lg:gap-8">
            ...
            <main className="flex-1 w-full flex flex-col gap-4 sm:gap-6 lg:gap-8 relative z-10 min-w-0">
              ...
              {/* HERO CAROUSEL */}
              <div 
                className="w-full rounded-xl sm:rounded-2xl lg:rounded-3xl relative overflow-hidden flex items-center h-[180px] sm:h-[250px] lg:h-[350px] border border-gray-200 dark:border-[#2A2A38] shadow-md sm:shadow-lg bg-gray-900 group"
              ...
              >
                  ...
                  <div key={index} className={`absolute inset-0 w-full h-full transition-opacity duration-1000 ${currentSlide === index ? 'opacity-100 z-10' : 'opacity-0 z-0'}`}>
                    ...
                    <div className="absolute inset-0 bg-gradient-to-r from-black/90 via-black/70 to-transparent flex items-center">
                      <div className="relative z-20 w-full p-4 sm:p-6 lg:p-14">
                        ...
                        <button className="bg-[#6324E2] hover:bg-[#501bb8] text-white font-bold text-[10px] sm:text-sm lg:text-base px-4 py-1.5 sm:px-6 sm:py-2.5 lg:px-8 lg:py-3 rounded-full transition-colors shadow-lg">
                          {slide.button}
                        </button>
                      </div>
                    </div>
                ...
                {/* Arrows — always visible, frosted glass */}
                <button 
                  onClick={prevSlide}
                  className="absolute left-2 sm:left-4 top-1/2 -translate-y-1/2 w-8 h-8 sm:w-10 sm:h-10 rounded-full bg-white/10 hover:bg-white/25 text-white flex items-center justify-center backdrop-blur-md transition-all z-20 shadow-lg border border-white/20"
                >
                  <ChevronLeft size={20} className="sm:w-6 sm:h-6" />
                </button>
                <button 
                  onClick={nextSlide}
                  className="absolute right-2 sm:right-4 top-1/2 -translate-y-1/2 w-8 h-8 sm:w-10 sm:h-10 rounded-full bg-white/10 hover:bg-white/25 text-white flex items-center justify-center backdrop-blur-md transition-all z-20 shadow-lg border border-white/20"
                >
                ...
                <div className="absolute bottom-2 sm:bottom-4 left-0 right-0 flex justify-center gap-1.5 sm:gap-2 z-30">
                  {HERO_SLIDES.map((_, idx) => (
                    <button key={idx} onClick={() => setCurrentSlide(idx)} className={`h-1.5 sm:h-2 lg:h-2.5 rounded-full transition-all ${currentSlide === idx ? 'bg-white w-4 sm:w-6 lg:w-8' : 'bg-white/50 w-1.5 sm:w-2 lg:w-2.5'}`}/>
                  ))}
                </div>
              </div>
              ...
              <div>
                <div className="flex items-center justify-between bg-red-500 rounded-t-lg sm:rounded-t-xl px-3 sm:px-4 py-2 sm:py-3">
                  <h2 className="text-xs sm:text-sm lg:text-base font-bold text-white flex items-center gap-1 sm:gap-2">
                    <Zap className="text-yellow-300 w-3 h-3 sm:w-4 sm:h-4 lg:w-[18px] lg:h-[18px]" fill="currentColor"/> Flash Sales
                  </h2>
                    ...
                    <Link to={`/product/${product.id}`} key={product.id} className="bg-white dark:bg-[#12121D] border border-gray-200 dark:border-[#2A2A38] hover:border-[#6324E2] dark:hover:border-[#6324E2] rounded-lg sm:rounded-xl flex flex-col group cursor-pointer transition-all overflow-hidden shadow-sm hover:shadow-xl h-full relative">
                      ...
                      <div className="p-1.5 sm:p-3 lg:p-4 flex flex-col flex-1">
                        ...
                        <div className="mt-auto">
                          <div className="font-black text-gray-900 dark:text-white text-[11px] sm:text-sm lg:text-base mb-1 sm:mb-2">{product.price}</div>
            ...
            <aside className="w-full xl:w-[320px] shrink-0 flex flex-col md:flex-row xl:flex-col gap-4 sm:gap-6 relative z-10">
              ...
              <div className="bg-white dark:bg-[#12121D] border border-gray-200 dark:border-[#2A2A38] rounded-xl sm:rounded-2xl p-4 sm:p-6 shadow-md flex-1 flex flex-col">
                <div className="flex justify-between items-center mb-4 sm:mb-5">
                  <h3 className="text-[10px] sm:text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider">RECENT TRANSACTIONS</h3>
                ...
                <div className="flex flex-col gap-3">
                  {RECENT_TRANSACTIONS.map((txn) => (
                    <div key={txn.id} className="flex items-center gap-3">
                      <div className={`w-7 h-7 sm:w-8 sm:h-8 rounded-full ${txn.bg} ${txn.color} flex items-center justify-center shrink-0`}>
                        {txn.icon}
                      </div>
                      <div className="flex-1 min-w-0">
                        <p className="text-[10px] sm:text-xs font-bold text-gray-800 dark:text-gray-200 truncate">{txn.label}</p>
                        <p className="text-[8px] sm:text-[9px] text-gray-400 dark:text-gray-500">{txn.date}</p>
                      </div>
                      <span className={`text-[10px] sm:text-xs font-black ${txn.color} shrink-0`}>{txn.amount}</span>
                    </div>
                  ))}
                </div>
              </div>
